...
// State
// State 주석 사항은 backend 제작 시 Saga 연동 / frontend 의 로딩화면 구축하기 위해 놔둔 사항
...
export const ToggleTrainingRequest = (
  date,
  routineIndex,
  trainingIndex,
  result
...
export const UpdateTrainingCountRequest = (
  date,
  routineIndex,
  trainingIndex,
  count
  ...
  return produce(state, draft => {
    switch (action.type) {
      ...
      case ADD_ROUTINE_SUCCESS: {
        ...
        if (draft.routine[action.data.date])
          draft.routine[action.data.date].push(action.data.object);
      ...
      case DELETE_ROUTINE_SUCCESS: {
        ...
        else draft.routine[action.data.date].splice(action.data.index, 1);
        localStorage.setItem("routine", JSON.stringify(draft.routine));
        break;
      }
      ...
      case TOGGLE_TRAINING_SUCCESS: {
        ...
        draft.routine[action.data.date][action.data.routineIndex].trainings[
          action.data.trainingIndex
        ].done = action.data.result;
        localStorage.setItem("routine", JSON.stringify(draft.routine));
        break;
      }
      ...
      case UPDATE_ROUTINENAME_SUCCESS: {
        ...
        draft.routine[action.data.date][action.data.routineIndex].routineName =
          action.data.routineName;
        localStorage.setItem("routine", JSON.stringify(draft.routine));
        break;
      }
      ...
      case ADD_MEMO_REQUEST: {
        // memoAdde, isMemoAdding 같은거 하나도 안 씀. 정리 해야함
      ...
      case ADD_MEMO_SUCCESS: {
        ...
        if (
          draft.memo[action.data.date] &&
          draft.memo[action.data.date].length > 0
        ) {
          const maxKey = draft.memo[action.data.date].reduce((acc, now) =>
            acc < now.key ? now.key : acc
          );
          draft.memo[action.data.date].push({
            key: maxKey + 1,
            contents: action.data.memoText
          });
        } else {
          draft.memo[action.data.date] = [
            { key: 0, contents: action.data.memoText }
          ];
        }
      ...
      case DELETE_MEMO_SUCCESS: {
        ...
        // 마지막이 아니라 index를 찾아서 삭제해야 한다면
          ...
          const index = draft.memo[action.data.date].findIndex(
            value => value.key === action.data.key
          );
