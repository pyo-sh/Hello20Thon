...
const VideoList = () => {
    ...
    const { videos, nextPageToken, lastSearch, selectVideo } = useSelector(state => state.youtube);
    ...
    return (
        <VideosBox>
        <div><b>{lastSearch}</b>의 검색결과</div>
        <Vlist onScroll={onScroll}>
            {videos ? videos.map(video => <VideoListItem key={video.id} video={video} check={selectVideo.id === video.id}/>) : null}
        </Vlist>
        </VideosBox>
    );
};
