...
const UserRecordWeight = () => {
    ...
    const renderWeight = () => {
        if(isClicked){
            return <WeightInput type="number" onPressEnter={onPressEnter} defaultValue={weight}/>
        }
        else    return <div className="Weight-Amount" onClick={onClickDiv}>{weight}</div>;
    };

    return (
        <UpperDiv>
            Weight: {renderWeight()}kg
        </UpperDiv>
    );
};
